...
public class WeightLogDAO {
    ...
    private static final String UPDATE_USER_WEIGHT_SQL =
            "UPDATE \"USER\" SET start_weight_kg = ? WHERE user_id = ?";
    ...
    public void insertWeightLog(WeightLog log) {
        ...
        try {
            ...
            try (PreparedStatement psLog = connection.prepareStatement(INSERT_LOG_SQL)) {
                psLog.setInt(1, log.getUserId());
                psLog.setDate(2, log.getLogDate());
                psLog.setBigDecimal(3, log.getCurrentWeightKg());
                psLog.executeUpdate();
            }

            try (PreparedStatement psUser = connection.prepareStatement(UPDATE_USER_WEIGHT_SQL)) {
                psUser.setBigDecimal(1, log.getCurrentWeightKg());
                psUser.setInt(2, log.getUserId());
